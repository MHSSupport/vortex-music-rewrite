...
const chalk = require("chalk"),
	{ blueBright, cyanBright, greenBright, grey, magenta, magentaBright, redBright, yellow } = chalk,
...
class Logger {
	static log(content, options = {}) {
		...
		content = content.replace(new RegExp(process.env.PWD, "g"), ".").replace(new RegExp(token, "g"), "T0K3N");
		content.split("\n").forEach(sub => {
