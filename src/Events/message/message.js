			...
			if (command.args && !args.length) {
				return message.reply(
					`Sorry, this command requires arguments to function. Usage: ${
						command.usage ? `${this.client.prefix + command.name} ${command.usage}` : "This command doesn't have a usage format"
					}`
				);
			}
			...
			command.run(message, args).catch(error => {
				...
				message.channel.send(
					[
						...
						"For more support, please join our support server at https://discord.gg/KFVkKYDb2P"
					].join("\n")
				);
